...
import controllers.common.QuestionController;
...
public class ComprehensionQuestionGenerator {
	public static Project project = Project.getProject("Passages");
	public static QuestionType questionType = QuestionType.getType("Comprehension");


	// Lesson identification globals
	public static String globalPrompt;
//	public static long globalModId;
//	public static Exercise globalMod;
	public static User globalUser;

	public ComprehensionQuestionGenerator(){}
	private static ArrayList<Actor> actorMatrix;
	private static HashMap<Fact, Short> factFrequencyMatrix;

	// Temporary fake distractors
	public static String[] fakeNames = {"Brandon", "Molly", "Jeff", "Mike", "Ray", "Aakash", "Ellie", "Christina", "Tim", "Rose", "Monica", "Kayleigh", "Ian", "Walker", "Alicia", "Grace"};
	public static String[] fakePlaces = {"India", "Disney World", "Japan", "Korea", "Africa", "D.C.", "Philadelphia", "Michigan", "Geneva", "Ukraine", "Egypt", "Canberra", "Laos"};
	...
	private static String serializedClassifier = "lib/stanford-ner-2015-12-09/classifiers/english.all.3class.distsim.crf.ser.gz";
	...
	//TODO: question group
//	private static QuestionGroup qg;
//	private static List<QuestionPart> questions;
	...
	public static Fact extractOneFact(CoreMap mySentence) {
		...
		Tree subjectTree = null, predicateTree = null, objectTree = null, indirectObjectTree = null, complementTree = null;
	 ...
	 * @return
	 */
	public static void generateComplexChooseQuestions(int n) {
		...
		for (int x = 0; x < n; x++) {
			...
			// Grab the correct answer (the thing actor 1 did NOT do)
...
//			QuestionPart part = new QuestionPart(prompt);
//		    QuestionPart.create(part);
//			QuestionGroup.addQuestionPart(qg, part);
		    
		    // Add the correct choice (the thing the actor did NOT do)
//		    QuestionController.addChoice(part, correctFact.stringWithoutSubject(), true);
...
//				    QuestionController.addChoice(part, choiceText, false);
		        }
...
//			questions.add(part);
	 ...
	 * @return
	 */
	public static void generateSimpleQuestions() {
			...
			factLoop: for (Fact fact : actor.getFactMatrix()) {
				...
				if (actor.getProperNounType().equals("PERSON") && fact.getObject() != null && actorMatrix.size() > 2) {
					...
					if (distractors.size() > 2) {
				        ...
				        Prompt prompt = new Prompt(globalPrompt);
				        prompt.save();
...
//				        QuestionController.addChoice(part, corrChoice, true);
...
//				        	QuestionController.addChoice(part, distractors.get(index), false);
				        	index++;
				        }
...
//				        questions.add(part);
				...
				// 	    Message prompt = new Message(globalUser, Message.Type.QUESTION, question);
				// 	    prompt.save();

				//         QuestionPart part = new QuestionPart(prompt, User.byId("1"), false, false, null);
				//         QuestionPart.create(part);
				//         String corrChoice = "";
				//         for(Label test : object.getCore().yield()){
				// 			corrChoice = corrChoice + test.value();
			 ...
			 //            int index = 0;
				...
				//         // TODO: do not add every single distractor?
				// 		while(index < distractors.size()){
				//         	QuestionController.addChoice(part, distractors.get(index).name, false);
				//         	index++;
				//         }
	 ...
	 * @return
	 */
	public static void generateComplexAndQuestions(int n) {
		// Cannot generate questions if the matrix is smaller than 2 actors
		if (actorMatrix == null || actorMatrix.size() < 2) {
			return;
		}
...
//						if (ques.prompt.text.equals(question) || ques.prompt.text.equals
//								(questionReverse)) {
...
//				    QuestionController.addChoice(part, primaryActor.getName(), true);
				    ...
				    for (int j = 0; j < 3; j++) {
				        ...
				        if(chosen.contains((Integer)idx))
				        	...
				        	chosen.add((Integer)idx);
//				        	QuestionController.addChoice(part, distrActors.get(idx).getName(), false);
				        }
...
//			        System.out.println(questions.size());
			...
			identifyRandomly: for (int k = 0; k < 10; k++) {
				...
				// check if any other and question already used these two facts
//				for (QuestionPart ques : questions) {
//					if (ques.prompt.text.equals(question) || ques.prompt.text.equals(questionReverse)) {
//						continue questionLoop;
//					}
//				}
...
//		        System.out.println(questions.size());
	...
	/**
	 * Rank the generated questions and add the top n to the exercise.
	 * @param numQuestions - the number of questions we want actually added to the exercise.
	 * @return
	 */
	public static void rankAndAddQuestions(int numQuestions){
		//System.out.println("gets here" + numQuestions + " " + questions.size());
...
//		System.out.println("TOTAL QUESTIONS = " + questions.size());
		...
		//TODO rank the generated questions here and sort by rank. For now, randomly shuffle
//		Collections.shuffle(questions);
		for(int i = 0; i < numQuestions; i++){
//			globalMod.questions.add(questions.get(i));
//			globalMod.save();
			//System.out.println("ADDED Q'S");
		}
	}
	...
	public static boolean generateQuestions(User user, String passage, int numQuestions) {
...
//		Course course = Course.getCourseForExercise(globalModId);
		...
		// a comprehension question QuestionPart Group has a passage as Content and counts as a nontrivial group (not a wrapper for one QuestionPart)
		ContentText contentText = ContentText.create(new ContentText(passage, globalUser.id));
		// TODO: is global?
//		qg = QuestionGroup.create(new QuestionGroup("Answer the following questions.", contentText, questionType, globalUser, project, true, true));
		...
		// It would be cool to use the dependency parser, but we apparently can't through the pipeline: http://stackoverflow.com/questions/28303398/in-the-corenlp-pipeline-is-it-possible-to-use-the-coref-tool-dcoref-with-the
		props.put("annotators", "tokenize, ssplit, pos, lemma, ner, parse, mention, dcoref"/*, depparse"*/);
				
		props.setProperty("dcoref.postprocessing", "true");
		/*props.setProperty("dcoref.score", "true");
		props.setProperty("dcoref.maxdist", "-1");
		props.setProperty("dcoref.use.big.gender.number", "false");
		props.setProperty("dcoref.replicate.conll", "true");
		//props.setProperty("dcoref.conll.scorer", "true");*/
		...
		/***** Example sentences *****/
		//annotation = new Annotation("Yesterday John sent an email to Stanford University. He didn't get a reply.");
		//annotation = new Annotation("John has become a regular visitor to a suburban garden.");
		//annotation = new Annotation("The board considered the proposal useless.");
		//annotation = new Annotation("He sent the girl a present.");
		//annotation = new Annotation("The little boy was happy.");
		//annotation = new Annotation("The black dog took a nap on the roof.");
		//annotation = new Annotation("Florida is nice.");
		//annotation = new Annotation("Katie is happy");
		//annotation = new Annotation("Ethan took a nice long nap on the couch.");
		//annotation = new Annotation("Before noon on Saturday, John saw Sam at the garden");
		//annotation = new Annotation("John had a very nice lunch.");// and John saw Sam at the garden");
		...
		generateComplexChooseQuestions(1);
		generateComplexAndQuestions(1);
		generateSimpleQuestions();

		rankAndAddQuestions(numQuestions);
		return true;
	}
}